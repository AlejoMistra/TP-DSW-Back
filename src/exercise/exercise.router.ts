...
import { CreateExerciseSchema, UpdateExerciseSchema, ExerciseIdSchema } from "./exercise.schemas.js";
import { z } from 'zod';
...
exerciseRouter.get('/:id', async (req: Request, res: Response) => {
    try {
        ...
        if (error instanceof z.ZodError) {
            return res.status(400).json({ message: 'Parámetros inválidos', errors: error.issues });
        }
    ...
    try {
        ...
        if (error instanceof z.ZodError) {
            return res.status(400).json({ error
                : 'Validación fallida', details: error.issues });
        }
